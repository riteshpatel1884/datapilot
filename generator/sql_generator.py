...
# def generate_sql(resolved_intent: str, run_config: dict = None) -> GeneratorResult:
#     """
#     run_config: optional dict passed to the chain's .invoke(config=...).
#     See ambiguity_classifier.classify() for why this exists — same
#     mechanism, used identically by the eval harness.
#     """
...
SYSTEM_PROMPT = """You are a SQL generator for a {dialect_name} database.
You will be given ONLY the schema sections and example queries that were
retrieved as relevant to this specific question (via RAG) — not the full
database schema. Use only what's given here.
...
- Use only tables/columns shown in the retrieved schema. Never invent columns.
- Use the retrieved examples as style/pattern guides, not literal answers —
  they may use different table/column names or even a different SQL
  dialect's date/string functions than what you're targeting now. Adapt
  the PATTERN (aggregation, filtering, join shape), never copy syntax
  that doesn't belong to {dialect_name}.
- Write syntax valid for {dialect_name} specifically (e.g. date/time
  functions, LIMIT/OFFSET, quoting) — not a different SQL dialect's syntax.
- Always include a LIMIT clause (default 100 if not specified).
- Respond ONLY with strict JSON, no markdown:
{{
  "sql": "SELECT ...",
...
def _mock_generate_for_connected_db(resolved_intent: str, session_id: str) -> GeneratorResult:
    """
    Mock-mode fallback when connected to a real (non-demo) database.
    The demo few-shot templates hardcode customers/orders table and
    column names, so adapting them here would confidently generate SQL
    against tables that don't exist in whatever the user connected.
    Instead: a simple keyword-overlap match against the CONNECTED
    database's actual table/column names — much cruder than an LLM,
    but at least targets real tables. Confidence is always reported
    "low" so the UI doesn't overstate how much to trust it.
    """
    from db.connection_manager import get_session

    session = get_session(session_id)
    schema = session.schema_cache
    lowered = resolved_intent.lower()
    words = set(re.findall(r"[a-z0-9_]+", lowered))

    best_table, best_score = None, 0
    for table, info in schema.items():
        table_words = {table.lower()} | {c[0].lower() for c in info["columns"]}
        score = len(words & table_words) + sum(1 for w in words if w in table.lower())
        if score > best_score:
            best_table, best_score = table, score

    if not best_table:
        # nothing matched — surface the actual table names so the person
        # knows what to ask about, rather than guessing at one
        table_list = ", ".join(sorted(schema.keys())[:10])
        return GeneratorResult(
            "", "low",
            f"No GROQ_API_KEY set, so mock mode can't reliably generate SQL for a connected "
            f"database — couldn't match your question to a table by keyword either. "
            f"Tables available: {table_list}",
        )

    count_signal = any(p in lowered for p in ["how many", "count of", "number of", "total number"])
    sql = f"SELECT COUNT(*) AS row_count FROM {best_table}" if count_signal else f"SELECT * FROM {best_table} LIMIT 100"

    return GeneratorResult(
        sql, "low",
        f"Mock mode (no GROQ_API_KEY): keyword-matched to table '{best_table}' — add a real "
        f"API key for actual question understanding against your connected database.",
    )
...
_DIALECT_DISPLAY_NAMES = {"postgres": "PostgreSQL", "sqlite": "SQLite"}


def generate_sql(resolved_intent: str, run_config: dict = None, session_id: str = None) -> GeneratorResult:
    """
    run_config: optional dict passed to the chain's .invoke(config=...).
    See ambiguity_classifier.classify() for why this exists — same
    mechanism, used identically by the eval harness.
    ...
    real PostgreSQL database, both the retrieved schema context AND
    the system prompt's target dialect switch to Postgres — see
    schema.schema_rag.retrieve_context() and db.connection_manager.get_dialect().
    """
    dialect = "sqlite"
    if session_id:
        from db.connection_manager import get_dialect
        dialect = get_dialect(session_id)
    dialect_name = _DIALECT_DISPLAY_NAMES.get(dialect, dialect)

    context = retrieve_context(resolved_intent, session_id=session_id, k_schema=3, k_examples=3)

    if MOCK_MODE:
        if dialect == "postgres":
            return _mock_generate_for_connected_db(resolved_intent, session_id)
        return _mock_generate(resolved_intent, context["examples"])
