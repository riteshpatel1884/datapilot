...
def execute_query(sql: str, db_path: str = DB_PATH, session_id: str = None) -> ExecutionResult:
    """
    If session_id is given and that session is connected to a real
    PostgreSQL database, routes execution there instead of the demo
    SQLite file — same validated-SQL-only contract, just a different
    backend. Demo-mode behavior (session_id=None, or a session that's
    never connected) is completely unchanged below.
    """
    if session_id:
        from db.connection_manager import get_session, POSTGRES_MODE
        session = get_session(session_id)
        if session.mode == POSTGRES_MODE:
            from db.postgres_connector import execute_query_postgres
            return execute_query_postgres(session.engine, sql)
