...
//           <div className="rail-section rail-buttons">
//             <button className="rail-trigger-button" onClick={() => setSchemaOpen(true)} type="button">
//               <span className="rail-trigger-icon" aria-hidden="true">⌗</span>
//               View database schema
//             </button>
//             <button className="rail-trigger-button" onClick={() => setSuggestOpen(true)} type="button">
//               <span className="rail-trigger-icon" aria-hidden="true">✦</span>
...
//                   <p><strong>Customers</strong> — name, city, signup date.</p>
//                   <p><strong>Orders</strong> — item purchased, category, amount spent, order date.</p>
//                 </div>
//               </details>
...
//                   Browse <button type="button" className="empty-state-link" onClick={() => setSuggestOpen(true)}>example questions</button>, or type your own above.
//                 </p>
//               </div>
//             )}
//             {history.map((item, i) => (
...
//       <SuggestionsPopup open={suggestOpen} onClose={() => setSuggestOpen(false)} onSelect={(q) => handleAsk(q)} />
//     </main>
//   );
// }
...
//           <p className="status-detail">Can&apos;t reach <code>{API_URL}</code>. Is FastAPI running?</p>
//         </div>
//       </div>
//     );
//   }
...
//   const stoppedAtLabel = failed ? STAGES[reached] : null;
...
//         <span className="card-status" data-state={failed ? "failed" : "answered"}>
//           {failed ? "Not answered" : "Answered"}
//         </span>
//       </div>
...
//             {result.message}
//           </div>
//           {stoppedAtLabel && <p className="card-stopped-at">Stopped at {stoppedAtLabel}</p>}
//         </>
//       )}

//       {result.type === "result" && (
//         <>
//           <p className="answer-statement">{result.summary}</p>

//           {result.table && result.table.length > 0 ? (
//             <ResultTable rows={result.table} />
//           ) : (
//             <p className="no-rows">No rows returned.</p>
//           )}

//           <details className="sql-details">
//             <summary className="sql-summary">
//               <span className="sql-summary-label">SQL</span>
...
export default function Home() {
  ...
  async function checkDbStatus(sid) {
    try {
      const res = await fetch(`${API_URL}/db/status?session_id=${encodeURIComponent(sid)}`);
      const data = await res.json();
      setDbStatus(data);
    } catch {
      setDbStatus({ mode: "demo", connected: true });
    }
  }
  ...
  async function callPipeline(query, selectedOption) {
    const res = await fetch(`${API_URL}/query`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ query, selected_option: selectedOption || null, session_id: sessionId }),
    });
    if (!res.ok) throw new Error(`API error: ${res.status}`);
    return res.json();
  }
    ...
    <main className="home">
      ...
      <section className="console">
        <div className="console-rail">
          ...
          <div className="rail-section rail-buttons">
            <button className="rail-trigger-button" onClick={() => setDbPanelOpen(true)} type="button">
              <span className="rail-trigger-icon" aria-hidden="true">
                <span className="db-status-dot" data-state={dbStatus?.mode === "postgres" ? "postgres" : "demo"} />
              </span>
              {dbStatus?.mode === "postgres" ? "Connected: PostgreSQL" : "Database: Demo"}
            </button>
            <button className="rail-trigger-button" onClick={() => setSchemaOpen(true)} type="button">
              <span className="rail-trigger-icon" aria-hidden="true">⌗</span>
              View database schema
      ...
      <SchemaPanel open={schemaOpen} onClose={() => setSchemaOpen(false)} sessionId={sessionId} dbMode={dbStatus?.mode} />
      <SuggestionsPopup open={suggestOpen} onClose={() => setSuggestOpen(false)} onSelect={(q) => handleAsk(q)} />
      <DbConnectPanel
        open={dbPanelOpen}
