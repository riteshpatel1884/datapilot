...
def get_full_schema(db_path: str = DB_PATH, session_id: str = None) -> dict:
    """
    Ground-truth schema — used by the validator too.

    If session_id is given and that session is connected to a real
    PostgreSQL database (see db/connection_manager.py), returns that
    session's introspected schema instead of reading the demo SQLite
    file. Demo-mode behavior (session_id=None, or a session that's
    never connected to Postgres) is completely unchanged below.
    """
    if session_id:
        from db.connection_manager import get_session, POSTGRES_MODE
...
def retrieve_context(query: str, session_id: str = None, k_schema: int = 3, k_examples: int = 3) -> dict:
    """
    Returns:
      {
        "schema_text": "...",       # relevant table docs, joined
    ...
    If session_id is given and connected to a real PostgreSQL database,
    schema retrieval runs against that session's own ephemeral,
    in-memory schema index instead of the demo collection — so a
    connected user's table/column names are what get retrieved, never
    the demo schema. The few-shot EXAMPLES collection is still shared
    globally either way: those are generic SQL style patterns ("count
    of X", "top N by Y"), not schema-specific facts, and the generator
    prompt already instructs the model to treat them as style
    references only, never literal answers — see generator/sql_generator.py.
    """
    if session_id:
        from db.connection_manager import get_session, POSTGRES_MODE
        session = get_session(session_id)
        if session.mode == POSTGRES_MODE:
            schema_text = ""
            if k_schema > 0 and session.schema_store is not None:
                hits = session.schema_store.similarity_search(query, k=k_schema)
                schema_text = "\n\n".join(doc.page_content for doc in hits)

            examples = []
            if k_examples > 0:
                _, examples_store = get_stores()
                hits = examples_store.similarity_search(query, k=k_examples)
                examples = [{"question": d.page_content, "sql": d.metadata.get("sql", "")} for d in hits]
