...
#   or SQL.
# - Every option must be a COMPLETE, directly resolvable interpretation —
#   something the SQL generator can turn straight into a query with no
#   further input needed. Never offer an option that itself requires a
#   missing value the user hasn't supplied yet (e.g. don't offer
#   "Customers from a specific city" as a clickable choice when you don't
#   know which city — that just produces SQL with an unfilled
#   placeholder). If the true ambiguity is "which specific value" (a
#   particular city, category, date range, etc.) rather than "which
#   interpretation", phrase clarify_question to explicitly ask the user to
#   name that value directly, and leave options empty so they answer in
#   free text instead of picking a template that can't be executed as-is.
...
# def classify(query: str, selected_option: str = None, run_config: dict = None) -> ClassifierResult:
#     """
...
#     run_config: optional dict passed straight through to the LangChain
#     chain's .invoke(config=...) — used by evaluation/run_eval.py to tag
#     each call with the eval case id and attach a token-usage callback,
#     so LangSmith traces are filterable per case and token/cost totals
#     can be aggregated locally without needing the LangSmith UI open.
#     Production calls (via pipeline.py -> api.py) simply don't pass this,
#     so nothing changes for normal app usage.
#     """
#     if selected_option:
...
def classify(query: str, selected_option: str = None, run_config: dict = None, session_id: str = None) -> ClassifierResult:
    """
    If selected_option is provided, the user already answered a
    clarification question — merge it into the intent directly instead
    of re-running the classifier.
    ...
    real PostgreSQL database, schema context comes from that database
    instead of the demo one — see schema.schema_rag.retrieve_context().
    """
    if selected_option:
        resolved = f"{query} (clarified: {selected_option})"
        return ClassifierResult(status="resolved", resolved_intent=resolved,
    ...
    # RAG: pull relevant schema so the LLM judges ambiguity against the real DB
    context = retrieve_context(query, session_id=session_id, k_schema=3, k_examples=0)
